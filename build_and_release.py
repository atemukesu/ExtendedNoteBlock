...
""")
        for mc_ver in ["1.20.1", "1.21.1"]:
            ...
            if os.path.isfile(jar_file):
                ...
                f.write(f"- `extendednoteblock-{VERSION}+{mc_ver}.jar` ({size})\n")
                f.write(f"  - MD5: `{md5}`\n")
...
def check_gh_auth():
    ...
    if ask_yes_no("是否现在进行 GitHub 登录？", default_yes=True):
        ...
        run_cmd(["gh", "auth", "login"], capture=False, check=False)
        result = run_cmd(["gh", "auth", "status"], capture=True, check=False)
        if "Logged in to" in result:
...
def upload_to_github():
    ...
    cmd = ["gh", "release", "create", release_tag,
           "--repo", gh_repo,
           "--title", f"ExtendedNoteBlock {release_tag}",
...
def upload_to_modrinth():
    ...
    if not modrinth_id or modrinth_id == "# Modrinth project slug or ID":
...
def build_version(mc_version):
    ...
    switch_task = f"switchTo{mc_version.replace('.', '')}"
...
def main():
    print("==============================================")
    print("  ExtendedNoteBlock Mod 自动化构建与发布工具")
    print("==============================================")
